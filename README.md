...
> **Ключевой результат:** лучшей конфигурацией в итоговой сводке стала `RidgeClassifier + TF-IDF с биграммами` с `Accuracy = 0.92` и `Macro F1 = 0.92`.
...
В папке `report/` находится **исходная PDF-версия курсовой работы** с кодом, результатами выполнения, таблицами, графиками и пояснениями.

Исходный `.ipynb` для этой курсовой среди доступных файлов не сохранился, поэтому в репозитории публикуется оригинальный PDF-отчёт без изменения содержимого.
